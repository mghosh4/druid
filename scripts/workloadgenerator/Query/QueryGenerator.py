...
class QueryGenerator(object):
    ...
    @staticmethod
    def getQueryType(querytype, queryratio):
    ...
    @staticmethod
    def generateQueries(startTime, endTime, numQueries, accessGenerator, periodGenerator, popularityList, querytype, queryratio, logger):
        querylist = list()
        ...
        for i in xrange(numQueries):
            ...
            qtype = QueryGenerator.getQueryType(querytype, queryratio);
            q.setInterval(startstring + "/" + Utils.iso8601(dt.timedelta(seconds=period)))
            q.setQueryType(qtype)
            querylist.append(q)
            logger.info("Query Details: " + q.getQueryType() + "," +  q.getInterval())
    ...
    @staticmethod
    def generateQueriesFromFile(startTime, endTime, numQueries, accessGenerator, periodGenerator, querytype, queryratio, filename):
        ...
        for i in xrange(numQueries):
            ...
            q.setQueryType(Query.getQueryType(querytype, queryratio))
            querylist.append(q)
            ##print "interval: " + q.interval
            ##print "index: " , q.index
        return querylist
